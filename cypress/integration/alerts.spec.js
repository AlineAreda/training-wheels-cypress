...
it('deve exibir uma mesagem de alerta', () => {
...
it('deve exibir mensagem de boas vindas com o nome', () => {
    cy.visit('/javascript_alerts')

    cy.window().then(function ($win) {
        cy.stub($win, 'prompt').returns('Aline')
        cy.contains('button', 'Prompt').click()
    })

    cy.get('#result').should('have.text', 'Olá, Aline')
